...
return function(opt)		
	...
	convLayer(net, opt.nDepth, 16, 3)
	convLayer(net, 16, 16, 3)
	convLayer(net, 16, 16, 3)
	net:add(nn.Reshape(16*finalSize*finalSize))
	
	local criterion = nil
	if opt.classify then 
		net:add(nn.Linear(16*finalSize*finalSize, opt.nClasses))
	...
	model.outSize = 16*finalSize*finalSize
